...
import { runMultiAgentAnalysis } from "../../scripts/multi-agent-analysis.ts";
  ...
  try {
    ...
    for (const stock of stocks) {
      try {
        ...
        console.error(`종목 분석 실패 (${stock.name}):`, error);
        // 다음 종목 계속 진행
      }
    }

    return new Response(
      JSON.stringify({ message: "Multi-Agent 분석 완료", stocks: stocks.length }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 200 }
    );
  } catch (error) {
    return new Response(
